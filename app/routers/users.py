...
async def create_user(user_create: UserCreate, db: AsyncSession = Depends(get_async_db)):
    result = await db.scalars(select(UserModel.email).where(UserModel.email == user_create.email))
    if result.first():
...
async def get_new_access_token(
    ...
    try:
        payload = jwt.decode(refresh_token_, SECRET_KEY, algorithms=[ALGORITHM])
        email: str | None = payload.get("sub")
        token_type: str | None = payload.get("token_type")
